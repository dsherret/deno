  ...
  Env: NodeEnv + 'static,
{
  ...
  let resolver = state.borrow::<NodeExtResolver<Env::Fs>>().clone();
...
where
  Env: NodeEnv + 'static,
{
  let require_resolver = state.borrow::<Rc<dyn RequireNpmResolver>>().clone();
  let ext_node_resolver = state.borrow::<NodeExtResolver<Env::Fs>>().clone();
  let fs = state.borrow::<Env::Fs>().clone();

  let pkg_path = if require_resolver
    .in_npm_package(&PathBuf::from(&modules_path))
  ...
  Env: NodeEnv + 'static,
{
  ...
  let resolver = state.borrow::<NodeExtResolver<Env::Fs>>().clone();
  let permissions = state.borrow_mut::<Env::P>();
  resolver.get_closest_package_json(
  ...
  Env: NodeEnv + 'static,
{
  ...
  let fs = state.borrow::<Env::Fs>().clone();
  let node_ext_resolver = state.borrow::<NodeExtResolver<Env::Fs>>().clone();
  let require_resolver = state.borrow::<Rc<dyn RequireNpmResolver>>().clone();
  let permissions = state.borrow_mut::<Env::P>();
  let pkg = PackageJson::load(
    &fs,
  ...
  if pkg.imports.is_some() {
    ...
    state.put(require_resolver); // todo(dsherret): why is this done?
    r
  } else {
    Ok(None)
  }
