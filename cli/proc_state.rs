...
impl ProcState {
  ...
  pub async fn prepare_module_load(
    ...
    let npm_package_req_batches = {
      ...
      graph_data
        ...
        .unwrap()?;
      graph_data.npm_package_req_batches().clone()
    };
    eprintln!("{:?}", npm_package_req_batches);

    if !npm_package_req_batches.is_empty() {
      self
        .npm_resolver
        .add_package_req_batches(npm_package_req_batches)
        .await?;
  ...
  pub async fn create_graph_with_loader(
    ...
    let package_req_batches = resolve_npm_package_req_batches(&graph);
    if !package_req_batches.is_empty() {
      self
        .npm_resolver
        .add_package_req_batches(package_req_batches)
        .await?;
    }
