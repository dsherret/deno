...
impl NpmResolution {
  ...
  pub async fn add_package_req_batches(
    &self,
    package_req_batches: NpmPackageReqBatches,
  ) -> Result<(), AnyError> {
    ...
    let snapshot = self
      .add_package_req_batches_to_snapshot(package_req_batches, snapshot)
      .await?;
  ...
  pub async fn set_package_reqs(
    ...
    let snapshot = self
      .add_package_req_batches_to_snapshot(
        vec![package_reqs.into_iter().collect()],
        snapshot,
      )
      .await?;
